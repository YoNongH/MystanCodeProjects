...
The backend code encode the Breakout Project from Noah Huang.
"""
...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH, paddle_height=PADDLE_HEIGHT,
        ...
        # Draw bricks
        self.total_brick = brick_rows*brick_cols  # Total brick generate.
        for i in range(brick_rows):
            for j in range(brick_cols):
                self.brick = GRect(brick_width, brick_height)
                self.brick.filled = True
                self.brick.fill_color = 'Red'  # Making different color of brick.
                if 4 > i >= 2:
    ...
    def ball_get_obj(self):
        hit = True
        for i in range(0, self.ball.width * 2, self.ball.width):
            for j in range(0, self.ball.width * 2, self.ball.width):
                probe = self.window.get_object_at(self.ball.x + i, self.ball.y + j)
                if hit:
                    if probe is not None:
                        if probe == self.paddle:
                            if self.__dy > 0:  # Avoid the ball sticking to the board
                                self.set_dy()
                        else:
                            self.set_dy()
