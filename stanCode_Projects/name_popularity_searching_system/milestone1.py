...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    # name_date = {name:{year:rank}}
    if name not in name_data:     # Name not exist
        name_data[name] = {year:rank}
    else:
        if year not in name_data[name]:     # Name exist in different year
            name_data[name][year] = rank
        else:     # Name exist in same year but rank is different
